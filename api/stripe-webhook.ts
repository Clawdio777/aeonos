...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Add credits to the PayGated key
  try {
  ...
  if (email) {
    try {
      ...
      await resend.emails.send({
        from:    "AEONOS <noreply@basechainlabs.com>",
        to:      email,
        subject: "Your AEONOS API Key",
        html: `
<p>Thanks for your purchase. Here is your AEONOS API key:</p>
...
<p style="color:#888;font-size:13px;">Credits are consumed per tool call. Check your balance at: <a href="${pgUrl}/keys/${apiKey}/balance">balance check</a></p>
      ...
      // Non-fatal — key is already credited
    }
  }
