...
const PACKS: Record<string, { credits: number; amountCents: number; label: string }> = {
  starter:  { credits: 1000,  amountCents: 1000,  label: "AEONOS Starter — 1,000 credits ($10)" },
  standard: { credits: 2750,  amountCents: 2500,  label: "AEONOS Standard — 2,750 credits ($25)" },
  pro:      { credits: 6000,  amountCents: 5000,  label: "AEONOS Pro — 6,000 credits ($50)" },
};
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Pre-generate a PayGated API key with 0 credits (credits added by webhook after payment)
  let apiKey: string;
  try {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    line_items: [{
      price_data: {
        currency: "usd",
        product_data: { name: selectedPack.label, description: "AEONOS credits are consumed per tool call. 1 credit = $0.01 USD." },
